...
export default function Contact(): JSX.Element {
  return (
    <>
      <section className='mx-auto min-h-screen w-fit space-y-6 py-20'>
        <h2 className='text-xl font-medium underline underline-offset-8 lg:text-4xl'>
          Contact-us
        </h2>
        <p className='py-4 text-sm xl:text-lg'>
        ...
        <div className='flex flex-wrap items-center gap-40'>
          <iframe
            className='aspect-video h-full w-full rounded border-none lg:h-1/2 lg:w-1/2'
...
const ContactDetailCard = ({
  ...
  return (
    <div className='grid grid-cols-[3.5rem_repeat(1,minmax(0,1fr))] grid-rows-2 items-center'>
      ...
      {type === 'Email' && (
        <a href={`mailto:${link}`} className='text-xs hover:underline'>
          {field}
        </a>
      )}
      {type === 'Phone Number' && (
        <a href={`tel:${link}`} className='text-xs hover:underline'>
          {field}
        </a>
      )}
      {type === 'Location' && (
