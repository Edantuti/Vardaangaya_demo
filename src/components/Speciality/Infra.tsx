                    ...
                    <ul className="flex flex-col gap-4 text-xl font-mono my-6 row-span-2">
                        {infraObjectList.map((obj)=>(
                            <li key={obj.id} className="flex flex-col">
                                ...
                                {obj.url && <Link to={`/infra/${obj.url}`} className="text-xs text-blue-600">Read more</Link>}
                            </li>
                        ))}
                    </ul>
                </div>
                <img src={OT} alt="gynaecology" className="h-auto w-full rounded mx-auto my-auto shadow-md shadow-neutral-400 row-span-2 order-1 md:order-2"/>
            </article>
        </>
    )
}