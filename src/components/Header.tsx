...
const Header = () => {
  ...
  return (
    <>
      <header className='relative flex justify-between p-2 md:p-0 md:py-5'>
        <div className='flex'>
          <img
            src={Logo}
            alt='Hospital Logo'
          ...
          <div className='ml-2 mt-auto'>
            <h1 className='text-sm font-bold uppercase md:text-2xl'>
        ...
        {!drawerState ? (
          <GiHamburgerMenu
            ...
            className='flex-2 absolute right-4 top-10 h-5 w-5 cursor-pointer md:top-16 xl:hidden'
          />
        ) : (
          <RiCloseFill
            onClick={() => changeDrawerState(!drawerState)}
            className='flex-2 fixed right-4 top-10 z-20 h-5 w-5 cursor-pointer md:top-16 xl:hidden'
          />
        )}
      </header>
